...
class RandomPassword {
    ...
    generate(len = 8, useLowercase = true, useUppercase = true, useNumbers = true, useSymbols = true) {
    ...
    generatePassword3(len = 8) {
        if (typeof len !== 'number') {
            throw new Error('Invalid length');
        }
        let useNumbers = this.getRandomBooleanWithRatio();
        let useSymbols = !useNumbers;
        return this.generatePassword(len,true,true,useNumbers,useSymbols)
    }
    getRandomBooleanWithRatio(ratio = 0.5) {
        ...
        return Math.random() < ratio;  // By default, the probability of returning true is 50%.
    }
    checkStrength(pwd) {
        let strength = new Strength(pwd);
        let status = strength.check();
        return status
    }
}
